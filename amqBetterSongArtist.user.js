...
async function changeMode(e) {
    if (e.altKey && e.key == 'g') {
        ...
        if (scoringMode === true) {
            ...
            doCORSRequest({
                method: 'get',
                url: `https://quiz-site-bice.vercel.app/api/autocomplete?type=uniqueArtists`,
                type: 'artists'
            });
        } else {
            doCORSRequest({
            ...
            doCORSRequest({
                method: 'get',
                url: `https://quiz-site-bice.vercel.app/api/autocomplete?type=artists`,
                type: 'artists'
            });
        }
    ...
    #answerResultsHelper = (value, hashesMap, scoreMap, answerMap, revealFunction) => {
        hashesMap.forEach((answer, sender) => {
            ...
            if (scoringMode === true) {
                ...
                let possibleAnswers = ''
                if (exceptionsList.includes(answerMap.get(sender).toLowerCase()) || titles.includes(answerMap.get(sender).toLowerCase())) {
