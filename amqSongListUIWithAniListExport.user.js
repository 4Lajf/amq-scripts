...
function addTableHeader() {
    ...
    let alIdCol = $(`<td class="alId"><b>AL ID</b></td>`);
...
function addTableEntry(newSong) {
    ...
    let alId = $(`<td class="alId"></td>`).text(newSong.siteIds.aniListId);
    ...
    authWindow.panels[0].panel
        .append($(`<div class="slAuthWindow"></div>`)
            ...
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
                    .append($(`<input id='slAppID' type='text'>`)
                    )
                )
            )
            ...
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
                    .append($(`<input id='slAppSecret' type='text'>`)
                    )
                )
            ...
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
                    .append($(`<input id='slAppCode' type='text'>`)
                    )
                )
                .append($(`<button id="slAuth" class="btn btn-primary songListOptionsButton" type="button">Authenticate</button>`).click(() => {
                    savedSettings.appSecret = slAuthWindow.querySelector('#slAppSecret').value
                    saveSettings();
                    authAL();
                }))
            )
...
function createWarningWindow() {
    ...
    warningWindow.addPanel({
        width: 1.0,
        height: 500,
        id: "slWarnWindow"
    });

    warningWindow.panels[0].panel
        .append($(`<div class="slWarnWindow"></div>`)
            .append($(`<span style="display: block; font-size: 30px; color: red; text-align: center"><b>RISK OF PERMAMENT DATA LOSS<br>
            THIS SCRIPT WILL WIPE YOUR LISTS<br>
            DO NOT LINK YOUR MAIN ACCOUNT<br>
            USE ONLY EMPTY ACCOUNTS</b></span>`))
            .append($(`<span style="display: block; text-align: center"><b>This script will keep your list clean, ensuring only animes that you didn't guessed are added to it<br>
            Type "I understand" to proceed.<br>
            This warning will only appear once</b></span>`))
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
                    .append($(`<input id='slWarningInput' type='text'>`)
                    )
                    ...
                    firstTimeUse = false;
                    savedSettings.firstTimeUse = false;
                    saveSettings();
                    warningWindow.close();
                    authWindow.open();
                }))
...
function mediaListInfo(username, state) {
    return new Promise(function (resolve, reject) {
        ...
        // open request
        xhr.open('POST', 'https://amq-proxy.herokuapp.com/https://graphql.anilist.co')
...
function authAL() {
    ...
    const json = {
        ...
        client_id: slAuthWindow.querySelector('#slAppID').value,
        client_secret: slAuthWindow.querySelector('#slAppSecret').value,
        code: slAuthWindow.querySelector('#slAppCode').value,
    }
...
function createSettingsWindow() {
    ...
    settingsWindow.panels[1].panel
        ...
        .append($(`<div class="slTableSettingsContainer"></div>`)
            ...
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
                    .append($("<input id='slShowAlId' type='checkbox'>")
                        .prop("checked", false)
                        .click(function () {
                            if ($(this).prop("checked")) {
                                $(".alId").show();
                            ...
                            savedSettings.alId = $(this).prop("checked");
                            saveSettings();
                        })
                    )
                    .append($("<label for='slShowAlId'><i class='fa fa-check' aria-hidden='true'></i></label>"))
                )
                .append($("<label>AL ID</label>"))
            )
            .append($(`<div class="slCheckbox"></div>`)
                .append($(`<div class="customCheckbox"></div>`)
...
function updateSettings() {
    ...
    $("#slShowAlId").prop("checked", savedSettings.alId);
...
function setup() {
    ...
    // Add metadata
    AMQ_addScriptData({
        name: "Song List UI with AniList Export",
        author: "TheJoseph98 & 4Lajf",
        description: `
            <p> Let's you export your wrong guessed anime to AniList so you can use them in your next game.</p>
