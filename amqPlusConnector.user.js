...
function setupListeners() {
    ...
    new Listener("answer results", (data) => {
        ...
        if (annSongId && songSourceMap.has(annSongId)) {
            ...
            const playerName = sourceInfo.username || sourceInfo.sourceInfo || 'Unknown';

            // Format message based on source type
            let message = '';
            if (sourceInfo.sourceInfo === 'Random' || sourceInfo.username === null) {
                message = '🎲 Song source: Random';
            } else {
                // Clean up player name - remove platform info if present
                const cleanName = playerName.replace(/\s*\([^)]+\)\s*$/, '').trim();
                message = `👤 Song source: ${cleanName}`;
            }

            console.log("[AMQ+] Song source:", message, "for annSongId:", annSongId);
